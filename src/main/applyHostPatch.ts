    ...
    try {
        ...
        for (let i = undo.length - 1; i >= 0; i--) {
            try {
                ...
                console.error("[Vault] Rollback step failed", cleanupErr);
            }
        }
        throw err;
    }
    ...
    try {
        for (const name of readdirSync(discordPath)) {
            ...
            try {
                ...
                console.error("[Vault] Skipping unreadable sibling", name, statErr);
                continue;
            }
            if (!isDir) continue;
        ...
        console.error("[Vault] Failed to scan for sibling versions", err);
        return null;
    }
