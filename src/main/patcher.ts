...
if (!IS_VANILLA) {
    ...
    if (!IS_VESKTOP && !IS_EQUIBOP) {
        try {
            ...
            console.error("[Vault] Failed to install host update hook", err);
        }
    }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Vault");
    ...
    console.log("[Vault] Running in vanilla mode. Not loading Vault");
}

console.log("[Vault] Loading original Discord app.asar");
